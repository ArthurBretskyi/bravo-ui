...
export default function getStoreTemplate(collectionTitle, generalApiOperation) {
  ...
  async function addItemWithCustomId({ id, data }) {
    if (!id || !data) throw new Error('addItemWithCustomId: id and data are required')
    currentItem.value = await generalApiOperation({
  ...
  async function subscribeToItemsList() {
    if (unsubscribe) unsubscribe() // щоб не дублювати
    unsubscribe = collectionDB.subscribeToList((data) => {
      itemsList.value = data
    })
  }

  function unsubscribeFromItemsList() {
    if (unsubscribe) {
      unsubscribe()
      unsubscribe = null
    }
  ...
  return {
    ...
    subscribeToItemsList,
    unsubscribeFromItemsList,
  }
}
